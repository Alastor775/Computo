...
void validarAdmi(){
    classDAO.LoginDAO obj = new classDAO.LoginDAO();
        ...
        if(obj.validar("select * from aministrador where usuario='"+Usuario1.getText()+"' and contrasena='"+Contra1.getText()+"'")==false){
...
void validarMaes(){
    classDAO.MaestroDAO mae = new classDAO.MaestroDAO();
        ...
        if(mae.validar("select * from docente where usuario='"+Usuario1.getText()+"' and contrasena='"+Contra1.getText()+"'")==false){
            JOptionPane.showMessageDialog(rootPane, "DATOS incorrectos");
        }else{
            java.awt.EventQueue.invokeLater(new Runnable() {
                public void run() {
